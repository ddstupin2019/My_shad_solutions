...
В рамках этого задания (и последующей большой домашки) таблица - это последовательность некоторых словарей,
где в роли строки таблицы выступает словарь, а ключ словаря задаёт колонку таблицы. 
(Таким образом, индекс в последовательности + ключ в словаре задают ячейку).
...
Операции над таблицами можно объявлять просто как функции, принимающие таблицу и
...
Объекты такого класса-наследника уже могут быть созданы как обычно. Зачастую абстрактные классы используются для описания т.н.
...
Note. Map в нашем задании не тождественнен функции `map` из python, которая реализует соответствие 1-к-1
(наша для каждой строки может вернуть любое число строк).
...
3. **`Join`** — наиболее сложная операция, объединяющая информацию из двух таблиц в одну по ключу.
...
(не нужно реализовывать синтаксис SQL, только концептуальную идею).
...
**Важное уточнение 2**: сделайте так, чтоб в джойнерах не было копипасты. Join должен быть ответственен за нахождение блоков с общим ключом - эта часть общая для любого вида джойна. А джойнер принимает два таких блока и объединяет их.
...
включить во вторую большую домашку (которая подразумевает код-ревью, если у вас будет желание), поэтому лучше пишите сразу хороший код :)
...
Вам может пригодиться [`https://docs.python.org/3.14/library/re.html#re.split`](https://docs.python.org/3.14/library/re.html#re.split).
Регулярное выражение для пробельных символов – `\s+`; а ещё с регулярками обычно хорошо помогают всякие LLM-ки
