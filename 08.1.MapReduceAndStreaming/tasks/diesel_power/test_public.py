...
class MapCase:
    ...
    ground_truth: list[operations.TRow]
    cmp_keys: tuple[str, ...]
    mapper_item: int = 0
...
MAP_CASES = [
    ...
    MapCase(
        mapper=operations.LowerCase(column='text'),
        data=[
    ...
    MapCase(
        mapper=operations.FilterPunctuation(column='text'),
        data=[
    ...
    MapCase(
        mapper=operations.Split(column='text'),
        data=[
            {'test_id': 1, 'text': 'one two three'},
    ...
    MapCase(
        mapper=operations.Split(column='text', separator=';'),
        data=[
            {'test_id': 1, 'text': 'one;two;three'},
            {'test_id': 2, 'text': 'tab;splitting;test'},
    ...
    MapCase(
        mapper=operations.Product(columns=['speed', 'distance'], result_column='time'),
        data=[
    ...
    MapCase(
        mapper=operations.Filter(condition=lambda row: row['f'] ^ row['g']),
        data=[
            {'test_id': 1, 'f': 0, 'g': 0},
            {'test_id': 2, 'f': 0, 'g': 1},
            {'test_id': 3, 'f': 1, 'g': 0},
    ...
    MapCase(
        mapper=operations.Project(columns=['value']),
        data=[
...
def test_mapper(case: MapCase) -> None:
    ...
    result = operations.Map(case.mapper)(iter(case.data))
...
REDUCE_CASES = [
    ...
    ReduceCase(
        reducer=operations.TopN(column='rank', n=3),
    ...
    ReduceCase(
        reducer=operations.TopN(column='rank', n=2),
        reducer_keys=('match_id',),
        data=[
    ...
    ReduceCase(
        reducer=operations.TermFrequency(words_column='text'),
        reducer_keys=('doc_id',),
        data=[
    ...
    ReduceCase(
        reducer=operations.Count(column='count'),
        reducer_keys=('word',),
        data=[
    ...
    ReduceCase(
        reducer=operations.Sum(column='score'),
        reducer_keys=('match_id',),
        data=[
...
def test_reducer(case: ReduceCase) -> None:
    ...
    result = operations.Reduce(case.reducer, case.reducer_keys)(iter(case.data))
    assert isinstance(result, tp.Iterator)
    assert sorted(result, key=key_func) == sorted(case.ground_truth, key=key_func)
...
@dataclasses.dataclass
class JoinCase:
    joiner: operations.Joiner
    join_keys: tp.Sequence[str]
    data_left: list[operations.TRow]
    data_right: list[operations.TRow]
    ground_truth: list[operations.TRow]
    cmp_keys: tuple[str, ...]
...
JOIN_CASES = [
    ...
    JoinCase(
        joiner=operations.InnerJoiner(suffix_a='_game', suffix_b='_max'),
        join_keys=('player_id',),
        data_left=[
            {'game_id': 2, 'player_id': 1, 'score': 17},
            {'game_id': 3, 'player_id': 1, 'score': 22},
...
def test_joiner(case: JoinCase) -> None:
    ...
    result = operations.Join(case.joiner, case.join_keys)(iter(case.data_left), iter(case.data_right))
    assert isinstance(result, tp.Iterator)
    assert sorted(result, key=key_func) == sorted(case.ground_truth, key=key_func)
...
@pytest.mark.parametrize('func_mapper, additional_memory', [
    (operations.DummyMapper(), 1 * MiB),  # Strange memory leap on test start
    (operations.LowerCase(column='data'), 500 * KiB),
    (operations.FilterPunctuation(column='data'), 500 * KiB),
    (operations.Split(column='data', separator='E'), 500 * KiB),
    (operations.Product(columns=['data', 'n'], result_column='prod'), 500 * KiB),
    (operations.Filter(condition=lambda row: row['data'] == 'HE.LLO'), 500 * KiB),
    (operations.Project(columns=['data']), 500 * KiB),
])
def test_heavy_map(func_mapper: operations.Mapper, additional_memory: int, baseline_memory: int) -> None:
    time.sleep(1)
    op = operations.Map(func_mapper)(get_map_data())
    run_and_track_memory(lambda: next(op), baseline_memory + additional_memory)


def test_heavy_split(baseline_memory: int) -> None:
    func_map = operations.Split(column='data', separator='E')
...
@pytest.mark.parametrize('func_reducer, additional_memory', [
    (operations.FirstReducer(), 500 * KiB),
    (operations.TermFrequency(words_column='key'), 500 * KiB),
    (operations.Count(column='key'), 500 * KiB),
    (operations.Sum(column='value'), 500 * KiB),
    (operations.TopN(column='key', n=5000), 2 * MiB),
])
def test_heavy_reduce(func_reducer: operations.Reducer, additional_memory: int, baseline_memory: int) -> None:
    op = operations.Reduce(func_reducer, ('key', ))(get_reduce_data())
...
@pytest.mark.parametrize('func_joiner, additional_memory', [
    (operations.InnerJoiner(), 100 * MiB),
    (operations.LeftJoiner(), 100 * MiB),
    (operations.RightJoiner(), 100 * MiB)
])
def test_heavy_join(func_joiner: operations.Joiner, additional_memory: int, baseline_memory: int) -> None:
    op = operations.Join(func_joiner, ('key', ))(get_reduce_data(), get_reduce_data())
    run_and_track_memory(lambda: next(op), baseline_memory + additional_memory)
...
@pytest.mark.parametrize('func_joiner', [
    ...
    operations.LeftJoiner(),
    operations.RightJoiner()
])
def test_complexity_join(func_joiner: operations.Joiner) -> None:
    list(operations.Join(func_joiner, ('key', ))(get_complexity_join_data(), get_complexity_join_data()))
