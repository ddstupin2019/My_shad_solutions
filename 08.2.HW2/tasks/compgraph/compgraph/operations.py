...
class Join(Operation):
    ...
    def __call__(self, rows: TRowsIterable, *args: tp.Any, **kwargs: tp.Any) -> TRowsGenerator:
        ...
        if len(args) == 0:
            raise Exception('args must have at least 1 argument')
...
class FilterPunctuation(Mapper):
    ...
    def __init__(self, column: str, punctuation: str = string.punctuation):
        """
        :param column: name of column to process
        :param maket: maket from filter
        """
        self._column = column
        self._maket = str.maketrans('', '', punctuation)

    def __call__(self, row: TRow) -> TRowsGenerator:
        result_row = copy.deepcopy(row)
        result_row[self._column] = row[self._column].translate(self._maket)
        yield result_row
